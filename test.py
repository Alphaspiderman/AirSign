...
def authenticate(user):
    ...
    for path in glob.glob(f"signatures/{user}/signature_*.png"):
        saved_img = cv2.imread(path)
        saved_features = extract_features(saved_img)
        score = cosine_similarity([login_features], [saved_features])[0][0]
        scores.append(score)
